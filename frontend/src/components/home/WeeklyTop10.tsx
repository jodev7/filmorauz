...
const hrefFor = (item: WeeklyTopItem) =>
  item.type === "series" ? `/series/${item.slug}` : `/movies/${item.slug}`;
...
// "Haftalik Top 10" — the week's most-watched titles (movies AND series) by
// view count. Top three stand on a cinema award podium (trophy + medals);
// ranks 4–10 sit in a glass number rail below.
export default function WeeklyTop10({ items }: WeeklyTop10Props) {
          ...
          {podium.map((item, i) => {
            ...
            return (
              <Link
                key={item.id}
                href={hrefFor(item)}
                className={`group flex w-[30%] max-w-[180px] min-w-0 flex-col items-center ${p.order}`}
              >
                <span
                ...
                >
                  ...
                  />
                  <span className="absolute top-1 left-1 flex items-center gap-0.5 rounded-md bg-black/60 px-1.5 py-0.5 text-[9px] font-medium text-white">
                    {item.type === "series" ? <Tv size={9} /> : <Film size={9} />}
