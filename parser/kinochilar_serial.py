...
class KinochilarSerialParser:
    def __init__(self) -> None:
        self._movie = KinochilarParser()
    ...
    def parse(self, url: str) -> Dict:
        logger.info(f"[KINOCHILAR SERIAL] parse start url={url}")
        ...
        for full_href, season, episode_no, label in ep_links:
            ...
            if key in grouped:
                continue
            try:
                logger.info(f"[KINOCHILAR SERIAL] fetching subpage S{season}E{episode_no} url={full_href}")
                ...
                logger.warning(f"[KINOCHILAR SERIAL] failed to fetch subpage S{season}E{episode_no}: {e}")
