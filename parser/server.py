...
def _run_uzmovi_browser_download(
    detail_url: str,
    job_id: str,
    output_name: str,
...
def _run_claimed_download(job: dict, parser_base_url: str):
    ...
    # uzmovi.net CDN signs every request with x-path / x-match headers
    # generated by the player JS. The browser downloader bypasses signing
    ...
    try:
        if source == "uzmovi":
            result = _run_uzmovi_browser_download(
                detail_url=referer or job.get("detail_url", ""),
                job_id=job_id,
                output_name=output_name,
                progress_state_cb=progress_callback,
            )
        else:
        ...
        # NEW: /download endpoint - starts non-blocking background download
            ...
            def background_download():
                try:
                    ...
                    if source == "uzmovi":
                        # uzmovi.net CDN segments are signed by the player JS;
                        # only the headless browser can pull them. The browser
                        # downloader bundles Playwright navigation + manifest
                        # capture + per-segment fetch + ffmpeg mux.
                        download_result = _run_uzmovi_browser_download(
                            detail_url=referer,
                            job_id=job_id,
                            output_name=output_name,
                            progress_state_cb=progress_callback,
                    ...
                    if not download_result.get("success"):
                        ...
                        # uzmovi failures don't have a fresh-URL retry path —
                        # the browser already re-loaded the page from scratch.
                        # Skip the auto-refresh branch for it.
                        if source == "uzmovi":
                            pass
                        # Auto-refresh expired URLs logic
                        elif source and source in PARSERS and referer:
