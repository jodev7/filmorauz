...
class FreekinoParser:
    ...
    def _extract_freekino_player(self, soup, base_url: str = ""):
        ...
        This handles the #2 base64-encoded hash format used by freekino.net.
        
        The video URL is dynamically generated by the player JavaScript which we can't
        easily replicate. Instead, we return the embed URL which the player uses.
        """
        entries = []
        quality_urls = {}
        ...
        for pattern in player_config_patterns:
            ...
            for match in matches:
                if not match.startswith("#2"):
                    continue

                encoded = match[2:]
                decoded = self._decode_freekino_hash(encoded)
                if decoded and "http" in decoded.lower():
                    quality_match = re.search(r'\[(\d+p)\]', decoded)
                    quality = quality_match.group(1) if quality_match else "720p"

                    url_match = re.search(r'(https?://[a-zA-Z0-9._~-]+(?:\.[a-zA-Z0-9._~-]+)+(?:/[^\s\[",\]]*)?)', decoded)
                    if url_match:
                        url = url_match.group(1)
                        if url and not url.startswith("blob") and "video" in url.lower():
                            vtype = "mp4"
                            entries.append({"url": url, "type": vtype, "quality": quality})
                            res = self._label_to_int(quality)
                            if res and res not in quality_urls:
                                quality_urls[res] = url

        if not entries:
            og_video = soup.find("meta", property="og:video:url")
            if og_video and og_video.get("content"):
                embed_url = og_video["content"]
                logger.info(f"[FREEKINO] og:video:url found - {embed_url}")
                entries.append({
                    "url": embed_url,
                    "type": "iframe_embed",
                    "quality": "auto"
                })
                logger.info(f"[FREEKINO] using embed URL as video URL: {embed_url}")
    ...
    def _decode_freekino_hash(self, encoded: str) -> str:
        """
        Decode the Freekino #2 hash format.
        The hash encodes video URLs like [720p]https://cdn.example.com/video.mp4
        """
        if not encoded:
            return ""
        ...
        for pad in range(3):
            try:
                ...
                if decoded and ("http" in decoded.lower() or ".mp4" in decoded or ".m3u8" in decoded):
                    return decoded
            except Exception:
                continue

        try:
            encoded_clean = re.sub(r'/+([A-Za-z0-9+]+)=*', r'/\1', encoded)
            ...
            logger.debug(f"[FREEKINO] hash decode by parts failed: {e}")
